<template>
  <section class="flex flex-col gap-8 grow p-8">
    <h1 class="text-xl font-bold text-white">Brouillons</h1>
      ...
      >
        <table class="w-full messages">
          ...
          <tbody>
            ...
            >
              <TableData class="text-white text-md">
        ...
        >
          <PageNumber route-name="app_draft_list" :query="route.query" />
          <Pagination :pagination="pagination"></Pagination>
        </div>
      </div>
      <div v-else class="grow flex flex-col justify-center items-center gap-2 py-32 text-center">
        <EmptyMessage :size="150" />
        <h2 class="text-xl font-semibold text-white">Aucun message brouillon pour le moment</h2>
...
const loadMessages = async () => {
  ...
  const response = await axiosInstance.get('/me/mailFolders/drafts/messages', {
    params: {
      $top: per_page.value,
      $skip: skip,
      $count: true,
      $select: 'id,subject,bodyPreview,toRecipients,isRead,receivedDateTime',
